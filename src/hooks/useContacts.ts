...
import { Contact, ContactRequest, Layer2Candidate } from "../types/contacts";
...
export function useLayer1Contacts() {
  const { user } = useAuth();
  const [contacts, setContacts] = useState<(Contact & { relationshipId?: string })[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(qAccepted, async (snapshot) => {
      try {
        const list: (Contact & { relationshipId?: string })[] = [];
...
export function useLayer2Contacts() {
  const { user } = useAuth();
  const [candidates, setCandidates] = useState<Layer2Candidate[]>([]);

  useEffect(() => {
    if (!user) return;
    getLayer2FromService(user.id).then(async (list) => {
      const formatted: Layer2Candidate[] = [];
      for (const item of list) {
        ...
        if (uSnap.exists()) {
          ...
          formatted.push({
            ...
            avatar: uData.avatar || uData.profile_photo || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde",
