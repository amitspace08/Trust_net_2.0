...
export async function sendSOSNotification(sender, receiver) {
...
export async function notifyLayer1(sessionId) {
  try {
...
export async function notifyLayer1FollowUp(sessionId) {
  try {
    const snap = await getDoc(doc(db, "sos_sessions", sessionId));
    if (!snap.exists()) return;
    const data = snap.data();
...
export async function notifyGuardian(sessionId, guardian, distressedUID) {
  try {
...
export async function notifyResponders(sessionId) {
  try {
...
export async function notifyNextCandidate(sessionId, nextCandidateUID, distressedUID) {
  try {
