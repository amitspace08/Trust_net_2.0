...
import {
  ...
  fetchPlaces,
} from "../services/safeSpaceService";
...
const EMPTY_FORM = { name: "", type: "shop", address: "", confirmed: false };
...
function SafeSpacesPage() {
  ...
  const [showMapPopup, setShowMapPopup] = useState(null);
  ...
  useEffect(() => {
    const fetchSpaces = async () => {
      try {
        ...
        const mappedList = firestoreSpaces.map((item) => ({
          id: item.id,
          name: item.name,
          type: item.type,
