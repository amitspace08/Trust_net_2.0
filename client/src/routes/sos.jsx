...
import { doc, onSnapshot, getDoc, GeoPoint } from "firebase/firestore";
...
const getRelativePercent = (lat, lng, centerLat, centerLng) => {
...
function SosEmergencyPage() {
  ...
  const [responderUID, setResponderUID] = useState(null);

  const [layer3Alerted, setLayer3Alerted] = useState([]);
  const [layer3Declined, setLayer3Declined] = useState([]);
  const [layer3Exhausted, setLayer3Exhausted] = useState(false);
  const [gaLocations, setGaLocations] = useState([]);
  ...
  const [gaRating, setGaRating] = useState(null);
  const [ratingSubmitted, setRatingSubmitted] = useState(false);
  const [gaSecondsLeft, setGaSecondsLeft] = useState(30);

  // Directions and Routes
  const [directionsToSafeSpace, setDirectionsToSafeSpace] = useState(null);
  ...
  useEffect(() => {
    if (!location || !nearestSpace || !window.google) return;
    if (lastRoutedLocation) {
      const dist = getDistance(
        location.lat,
        location.lng,
        lastRoutedLocation.lat,
        lastRoutedLocation.lng,
      );
      if (dist < 50) return;
    }
  ...
  useEffect(() => {
    if (!location || !responderLocation || !window.google) return;
    if (lastResponderRoutedLocation) {
      const dist = getDistance(
        responderLocation.lat,
        responderLocation.lng,
        lastResponderRoutedLocation.lat,
        lastResponderRoutedLocation.lng,
      );
      if (dist < 50) return;
    }
    const fetchRoute = async () => {
  ...
  // Layer 3 GA sequential timer countdown
  useEffect(() => {
    if (!isLayer3Escalated || isSafeEnded || responderName || layer3Exhausted) return;
    if (gaSecondsLeft <= 0) {
      // Automatically escalate to Police (Layer 4) when GA timer runs out
      const exhaust = async () => {
        try {
          ...
          setLayer3Exhausted(true);
          setActiveState("police-searching");
        } catch (err) {
          console.error("Failed to escalate to Police:", err);
        }
      };
      exhaust();
      return;
    ...
    return () => clearTimeout(timer);
  }, [isLayer3Escalated, isSafeEnded, responderName, gaSecondsLeft, layer3Exhausted, sessionId]);
  ...
  const submitGARating = async (score) => {
    if (!responderUID) return;
    try {
        ...
        {location ? (
          ...
          >
            ...
            <Circle
              center={[location.lat, location.lng]}
              radius={
                isLayer3Escalated ? pulseRadius : isLayer2Escalated ? pulseRadius : pulseRadius / 5
              }
              pathOptions={{
                color: isLayer3Escalated ? "#f59e0b" : isLayer2Escalated ? "#0d9488" : "#8b5cf6",
                fillColor: isLayer3Escalated
                  ? "#f59e0b"
                  : isLayer2Escalated
                    ? "#0d9488"
                    : "#8b5cf6",
            ...
            {nearestSpace && (
              <>
                <Circle
                  center={[nearestSpace.latitude, nearestSpace.longitude]}
                  radius={80}
                ...
                <Polyline
                  positions={[
                    ...
                    [nearestSpace.latitude, nearestSpace.longitude],
                  ]}
            ...
            {isLayer3Escalated &&
              gaLocations.map((ga) => {
                ...
                return (
                  <Marker
                    key={ga.uid}
                    position={[ga.latitude, ga.longitude]}
                    icon={isDeclined ? greyIcon : isActiveCandidate ? amberIcon : amberIcon}
                  >
                    <Tooltip
                      permanent
                      direction="top"
                    >{`GA — ${ga.name}${isDeclined ? " (No Response)" : ""}`}</Tooltip>
                  </Marker>
                );
              })}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p
                className={`mt-1.5 leading-relaxed font-semibold transition-all ${
                  layer3Exhausted
                    ? "text-xs text-emerald-50 font-bold"
